...
custom_checks:
  - name: total revenue non-negative
    query: |
      SELECT COUNT(*) FROM mart.sales_daily WHERE revenue < 0
    value: 0
@bruin */

WITH daily_orders AS (
  SELECT
    CAST(CAST(order_date AS TIMESTAMP) AS DATE) AS sale_date,
  ...
  SELECT
    CAST(CAST(o.order_date AS TIMESTAMP) AS DATE) AS sale_date,
